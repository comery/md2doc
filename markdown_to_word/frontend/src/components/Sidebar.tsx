      ...
      <div className="flex-grow p-3 overflow-y-auto">
        ...
        <div className="space-y-4">
            ...
            <section>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input type="text" id="font-en" value={config.fontFamily.english} onChange={(e) => handleConfigChange('fontFamily', 'english', e.target.value)} className="mt-1 block w-full px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                </div>
                <div>
                  <label htmlFor="font-cn" className="block text-sm font-medium text-gray-600 dark:text-gray-400">Chinese Font</label>
                  <input type="text" id="font-cn" value={config.fontFamily.chinese} onChange={(e) => handleConfigChange('fontFamily', 'chinese', e.target.value)} className="mt-1 block w-full px-3 py-1.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                </div>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="space-y-4">
                {(['h1','h2','h3','body'] as Array<keyof typeof config.fontSize>).map((key) => (
                  <div key={key}>
                    <label htmlFor={`fs-${String(key)}`} className="block text-xs font-medium text-gray-700 dark:text-gray-300 uppercase mb-1">{key}</label>
                    <input type="number" id={`fs-${String(key)}`} value={config.fontSize[key]} onChange={(e) => handleConfigChange('fontSize', key, parseInt(e.target.value, 10) || 0)} className="block w-full px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm" />
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="space-y-2">
                <div>
                    ...
                    <input type="color" id="bg-md" value={config.colors.markdownBg} onChange={(e) => handleColorChange('markdownBg', e.target.value)} className="mt-1 h-8 w-full block border-gray-300 dark:border-gray-600 rounded-md" />
                </div>
                <div>
                    <label htmlFor="bg-word" className="block text-sm font-medium text-gray-600 dark:text-gray-400">Word Panel</label>
                    <input type="color" id="bg-word" value={config.colors.wordBg} onChange={(e) => handleColorChange('wordBg', e.target.value)} className="mt-1 h-8 w-full block border-gray-300 dark:border-gray-600 rounded-md" />
                </div>
              </div>
            </section>
        </div>
